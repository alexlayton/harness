...
### AUTH-1 Device-flow polling — PARTIAL
- DONE: `openai_codex.rs:446 OAUTH_BODY_LIMIT`, `:448 read_bounded_body` used by `request_token_until:377` + `token_strict:349`; RFC8628 map (`pending→continue`, `slow_down→+5s`, `expired→Expired`, `denied→sanitized`) before status; `token_strict` 2xx-only no echo used by `refresh:213`/`login_browser:268`; cancel/expiry around sleep+request (`~303-326`, `:387,401 select!`).
- Tests `openai_codex.rs:735-810`: `device_poll_pending_then_success:814` (two direct `request_token` calls, not full `login_device` loop) PARTIAL; `device_poll_errors_map_before_status:853` DONE; `slow_down_increases_the_poll_interval:909` asserts `saturating_add(5)` only, no paused-time fixture PARTIAL; `malformed_and_oversized_bodies_are_rejected:933` DONE; `cancellation_aborts_sleep_and_request:965` pre-cancelled only, no in-loop/in-flight PARTIAL. MISSING: strict-status browser/refresh `400` test.

### AUTH-2 Refresh single-flight — PARTIAL
- DONE: `refresh_lock:openai_codex.rs:89, github_copilot.rs:806`; reload-recheck (`openai_codex:171-210`, `copilot:879-908`, short lock scopes; note Codex `&&` vs Copilot `||` divergence); `storage.rs:253 save_openai_codex_if_current`, `:333 save_copilot_if_current`, `:356 save_provider_value_if_current` CAS under `AuthFileLock`, callers fall back on `false`; no blocking mutex across net.
- MISSING: cross-process lease spanning reload→exchange→save (CAS-at-save only, no `lease` symbol).
- Tests: `github_copilot.rs:1176 concurrent_refresh_single_flights_on_one_network_call` uses fake `CountingClient` + manual lock, not real `ensure_valid` PARTIAL, no Codex counterpart; single-use rotating token all-succeed MISSING (fake reuses same token); persisted==newest slice DONE (`persisted.access==results[0]`, `storage.rs:~655 compare_and_save_rejects_stale`); two-store/process test MISSING.

### AUTH-3 Callback + Copilot login — PARTIAL
- DONE: `openai_codex.rs:468 CALLBACK_OVERALL_TIMEOUT 10m`, `:479 wait_for_callback→:482 with_idle(10s)`, `read_callback_head:531` 512B chunks through `\r\n\r\n`, `CALLBACK_HEAD_LIMIT:472 16KiB`; `is_callback_denial:584` + sanitized denial `:514`; `github_copilot.rs:554 login_with_events_and_persist` persist-before `MODEL_DISCOVERY_TIMEOUT:33 15s fetch_available_model_ids`, best-effort swallow; serial policy off path (`enable_known_models:464` not in login); `base_url_from_proxy_token:663`, `copilot_base_url:718`, `proxy_endpoint:443` HTTPS-only, reject userinfo/`/?#@`/bad port.
- Tests: `openai_codex.rs:1002 idle_connection_does_not_block_a_later_valid_callback` (idle+fragmented) DONE; `:990 callback_target_parsing_accepts_codes_and_denials` pure unit only, no live denial-terminates-promptly PARTIAL; `github_copilot.rs:1089,1105` proxy parse/reject DONE. MISSING: Copilot model-list-failure-still-persisted login test.
...
1. AUTH-1 full-loop + slow_down paused-time + in-flight cancel tests; AUTH-2 real single-flight + rotating-token + two-store tests (decide cross-process lease vs CAS-documented); AUTH-3 Copilot persist-survives-discovery-failure + live denial tests.
2. AGENT-3/AGENT-4 + PERF-1 missing tests; PERF-1 triple-scan fix.
3. TOOLS-3 shared-drain deadline test; TUI-2 metadata fit + width/combining/ZWJ property + height-clamp tests; AGENT-1/AGENT-2 deferred-sync/child-race tests.
4. PERF-5 benches (1k/10k) + external/replacement reconciliation; PERF-6 TUI quadratic guard (fix `lines().count()` walk, add bench).
5. CONFIG-1 nested-extra matrix + concurrent-mutator tests; CLEANUP-1 `cargo tree --duplicates` verify + remaining removals; CLEANUP-2/3/4 sweeps.
6. CI-1 workflow verify + Linux baseline; DOCS-1 contracts.
7. Re-run before handoff: `cargo fmt --all`, `cargo clippy --workspace --all-targets --locked -- -D warnings`, `cargo test --workspace --locked`, `cargo build --workspace --locked`, `cargo tree --workspace --duplicates`, `git diff --check`.
